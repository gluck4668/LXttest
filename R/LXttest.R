...
for(i in 1:nrow(df)){
  ...
  # 方差齐性分析  
   ...
   bar.test <- bartlett.test(test_data,test_group)
   bar.test.p <- bar.test[["p.value"]]
  ...
  equal_variance[i,ncol(equal_variance)] <- bar.test.p
  equal_variance[i,ncol(equal_variance)-1] <- t_test[["p.value"]]
  equal_variance[i,ncol(equal_variance)-3] <- sd(df[i,c(n1:n2)])
  equal_variance[i,ncol(equal_variance)-5] <- sd(df[i,c(1:group[1,2])])
 ...
 write.xlsx(equal_variance,"equal_variance_analysis.xlsx",rowNames=T)
